...
In order to migrate an app to use persistent queries, the queries of the current and previous client version should be persisted without hashing, so usage of these clients won't break, preventing problems with caching and removing the need to force updates to the user. It's possible by passing a function that returns the query without changes to `--persist-function`, and running the compiler for every version of the client app you wish to keep supporting.
